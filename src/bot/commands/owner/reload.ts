...
export default class ReloadCommand extends Command {
	...
	public async exec(
		...
		if (all) {
			try {
				const commands = await new Promise<number>(resolve => resolve(this.handler.reloadAll().modules.size));
				const listeners = await new Promise<number>(resolve =>
					resolve(this.client.listenerHandler.reloadAll().modules.size),
				);
				return msg.util?.reply(`successfully reloaded \`${commands}\` command and \`${listeners}\` listeners.`);
			} catch (err) {
